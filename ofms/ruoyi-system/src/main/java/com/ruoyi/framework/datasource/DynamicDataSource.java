...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    public boolean createDataSource(String key, String driveClass, String url, String username, String password, String databasetype) {
        System.out.println("createDataSource: url:"+url+", driveClass:"+driveClass+",databasetype:"+databasetype);
        try {
            try { // 排除连接不上的错误
...
//            log.info(key+"数据源的概况："+druidDataSource.dump());
    ...
    public boolean testDatasource(String key, String driveClass, String url, String username, String password) {
        ...
        System.out.println("createDataSource: url:"+url+", driveClass:"+driveClass);
    ...
    private  void createDataSource(DataSource dataSource) throws Exception {
        ...
        String databasetype = dataSource.getConnType();
        String username = dataSource.getConnUserName();
        String password = dataSource.getConnPassWord();
        String driveClass = dataSource.getConnDriverClass();
//        if("mysql".equalsIgnoreCase(databasetype)) {
//            driveClass = DBUtil.mysqldriver;
//        } else if("oracle".equalsIgnoreCase(databasetype)){
//            driveClass = DBUtil.oracledriver;
...
//        jdbc:microsoft:sqlserver://MyDbComputerNameOrIP:1433;databaseName=master
        ...
        if("mysql".equals(databasetype.toLowerCase())){
            url ="jdbc:mysql://"+ dataSource.getConnIp()+":"+dataSource.getConnPort()+"/"+dataSource.getConnDbName();
        }
...
//                throw new ADIException("数据源"+datasourceId+"配置正确，但是创建失败",500);
                ...
                log.error("数据源"+datasourceId+"创建成功！");
            }
        } else {
            log.error("数据源配置有错误");
